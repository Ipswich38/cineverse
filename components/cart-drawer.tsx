  ...
  return (
    <Sheet open={isOpen} onOpenChange={closeCart}>
      <SheetContent className="flex w-full flex-col gap-0 sm:max-w-md">
        ...
        {items.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-4 px-6 text-center text-[#6e6e73]">
            <div className="flex h-14 w-14 items-center justify-center rounded-full bg-[#f5f5f7]">
              <ShoppingBag className="h-7 w-7" />
            </div>
            <div>
              <p className="font-semibold text-[#1d1d1f]">Your cart is empty</p>
          ...
          <>
            <div className="flex-1 space-y-4 overflow-y-auto px-5 py-4">
                ...
                <div key={item.id} className="rounded-xl border border-black/[0.06] p-3">
                  <div className="flex gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {item.ownerName && <p className="text-[11px] text-[#6e6e73]">by {item.ownerName}</p>}
                      <p className="mt-0.5 text-[13px] text-[#6e6e73]">{formatMoney(item.price)}/day</p>
                    </div>
                    <button
                  ...
                  <div className="mt-3 flex items-center justify-between gap-2">
                    <MiniStepper
                      icon={<Calendar className="h-3 w-3 text-[#6e6e73]" />}
                      label="days"
                      value={item.days}
                    ...
                    />
                    <span className="ml-auto text-sm font-semibold text-[#1d1d1f]">{formatMoney(itemLineTotal(item))}</span>
                  </div>
                  ...
                  {item.operatorAvailable && (
                    <button
                      ...
                      className={`mt-2 flex w-full items-center gap-2 rounded-lg border px-2.5 py-2 text-left text-[12px] transition-colors ${
                        item.withOperator ? 'border-[#0071e3] bg-[#0071e3]/[0.04] text-[#0071e3]' : 'border-black/[0.08] text-[#6e6e73] hover:border-black/20'
                      }`}
                    >
                      ...
                      <span className={`flex h-4 w-4 items-center justify-center rounded border ${item.withOperator ? 'border-[#0071e3] bg-[#0071e3] text-white' : 'border-black/20'}`}>
              ...
              {recommendations.length > 0 && (
                <div className="rounded-xl border border-[#0071e3]/15 bg-[#0071e3]/[0.04] p-3">
                  <p className="mb-2 text-sm font-semibold text-[#1d1d1f]">Smart add-ons for your shoot</p>
                  <div className="space-y-2">
                    {recommendations.map((product) => (
                      <div key={product.id} className="flex items-center gap-3 rounded-lg bg-white p-2">
                        <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded bg-[#f5f5f7]">
                          <Image src={product.image_url} alt={product.name} fill className="object-cover" />
                        </div>
                        <div className="min-w-0 flex-1">
                          <p className="line-clamp-1 text-xs font-semibold text-[#1d1d1f]">{product.name}</p>
                          <p className="text-xs text-[#6e6e73]">{formatMoney(product.price)}/day</p>
                        </div>
                        <Button
                          size="sm"
                          variant="outline"
            ...
            <div className="space-y-3 border-t border-black/[0.06] p-5">
              <div className="space-y-1.5 text-sm text-[#6e6e73]">
              ...
              <div className="flex justify-between rounded-lg bg-[#0071e3]/[0.06] px-3 py-2.5 text-[15px] font-semibold text-[#0071e3]">
              ...
              <Link
                href="/checkout"
                onClick={closeCart}
                className={buttonVariants({ size: 'lg', className: 'h-11 w-full bg-[#1d1d1f] text-white hover:bg-[#1d1d1f]/85' })}
              >
...
function MiniStepper({
  ...
  return (
    <div className="flex items-center gap-1 rounded-lg bg-[#f5f5f7] px-1.5 py-1">
      <button onClick={onDec} className="flex h-6 w-6 items-center justify-center rounded text-[#1d1d1f] hover:bg-black/5" aria-label={`Decrease ${label}`}>
        <Minus className="h-3 w-3" />
      </button>
      <span className="flex min-w-[44px] items-center justify-center gap-1 text-[12px] font-semibold text-[#1d1d1f]">
        {icon}
        {value} {label}
      </span>
      <button onClick={onInc} className="flex h-6 w-6 items-center justify-center rounded text-[#1d1d1f] hover:bg-black/5" aria-label={`Increase ${label}`}>
